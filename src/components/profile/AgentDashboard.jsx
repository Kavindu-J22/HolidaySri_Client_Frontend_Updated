    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 w-full overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-4 sm:space-y-0 w-full">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 w-full sm:w-auto flex-shrink-0">
            {/* Claim Earnings Button */}
            <button
              onClick={() => navigate('/hsc')}
              className="flex items-center justify-center space-x-2 px-4 py-2 bg-gradient-to-r from-orange-500 to-yellow-500 hover:from-orange-600 hover:to-yellow-600 text-white rounded-lg transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 w-full sm:w-auto text-sm sm:text-base"
            >
              <Gift className="w-4 h-4 flex-shrink-0" />
              <span className="whitespace-nowrap">Claim Earnings</span>
            </button>
            ...
            {!showEarnings ? (
              <button
                ...
                className="flex items-center justify-center space-x-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50 w-full sm:w-auto text-sm sm:text-base"
              >
                {loadingEarnings ? (
                  <>
                  ...
                  <>
                    ...
                    <span className="whitespace-nowrap">View Records</span>
                  </>
                )}
              </button>
            ) : (
              <button
                ...
                className="flex items-center justify-center space-x-2 px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors w-full sm:w-auto text-sm sm:text-base"
              >
                <span className="whitespace-nowrap">Hide Records</span>
              </button>
            )}
          </div>
        </div>
        ...
        {showEarnings && (
          <div className="space-y-4">
            {/* Pagination Info */}
            {earningsTotal > 0 && (
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-4 space-y-1 sm:space-y-0">
                <span>
              ...
              <table className="w-full min-w-full">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700">
                    <th className="text-left py-3 px-4 font-semibold text-base text-gray-900 dark:text-white">
                      Buyer
                    </th>
                    <th className="text-left py-3 px-4 font-semibold text-base text-gray-900 dark:text-white">
                      Item
                ...
                <tbody>
                  {earningsData.map((earning) => (
                    <tr key={earning.id} className="border-b border-gray-100 dark:border-gray-700">
                      <td className="py-3 px-4 text-base text-gray-900 dark:text-white">
                        <div>
                      ...
                      <td className="py-3 px-4 text-base text-gray-600 dark:text-gray-400">
                        {earning.item}
                      </td>
                      <td className="py-3 px-4 text-base font-semibold text-gray-900 dark:text-white">
            ...
            {/* Mobile/Tablet Cards - Show on small and medium screens */}
            <div className="lg:hidden space-y-3 w-full">
              {earningsData.map((earning) => (
                <div key={earning.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 sm:p-4 w-full overflow-hidden">
                  ...
                  <div className="flex justify-between items-start gap-2 mb-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 break-all mt-0.5">
                        {earning.maskedEmail}
                      </p>
                    </div>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(earning.status)} flex-shrink-0 whitespace-nowrap`}>
                      {earning.status}
                    </span>
                  </div>
            ...
            {earningsPage < earningsTotalPages && (
              <div className="flex justify-center mt-4 sm:mt-6">
                <button
                  ...
                  className="flex items-center justify-center space-x-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base w-full sm:w-auto"
                >
                  {loadingMoreEarnings ? (
                    <>
                      <Loader className="w-4 h-4 animate-spin" />
                    ...
                    <>
                      <span className="hidden sm:inline">Show More ({earningsTotal - earningsData.length} remaining)</span>
                      <span className="sm:hidden">Show More ({earningsTotal - earningsData.length})</span>
