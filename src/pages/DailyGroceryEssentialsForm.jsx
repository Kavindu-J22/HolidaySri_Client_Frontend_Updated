...
const DailyGroceryEssentialsForm = () => {
  ...
  useEffect(() => {
    const fetchProvinces = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/daily-grocery-essentials/provinces');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/daily-grocery-essentials/publish', {
        method: 'POST',
        headers: {
