...
const TravelSafeHelpProfessionalsBrowse = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch provinces
        const provincesRes = await fetch('http://localhost:5000/api/travel-safe-help-professional/provinces');
  ...
  const fetchProfessionals = async (filterParams = filters, search = searchTerm) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/travel-safe-help-professional/browse?${queryParams}`);
