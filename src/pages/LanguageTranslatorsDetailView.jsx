...
const LanguageTranslatorsDetailView = () => {
    ...
    const fetchProfile = async () => {
      try {
        const [profileRes, reviewsRes] = await Promise.all([
          fetch(`http://localhost:5000/api/language-translators/${id}`),
          fetch(`http://localhost:5000/api/language-translators/${id}/reviews`)
        ]);
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/language-translators/${id}/reviews`, {
        method: 'POST',
        headers: {
