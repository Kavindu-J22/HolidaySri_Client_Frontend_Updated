...
const ClientBookingCard = ({ booking, onUpdate }) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/room-bookings/${booking.bookingId}/approve`, {
        method: 'PUT',
        headers: {
  ...
  const handleReject = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/room-bookings/${booking.bookingId}/reject`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
...
const HotelsAccommodationsBrowse = () => {
    ...
    if (activeTab === 'browse') {
      const fetchHotels = async () => {
        ...
        try {
          ...
          const response = await fetch(`http://localhost:5000/api/hotels-accommodations/browse?${params}`);
          const data = await response.json();
  ...
  useEffect(() => {
    const checkHotelOwner = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('http://localhost:5000/api/room-bookings/all-client-requests', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  const fetchMyBookings = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/room-bookings/my-bookings', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
