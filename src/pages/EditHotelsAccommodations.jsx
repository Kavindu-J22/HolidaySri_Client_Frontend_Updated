      ...
      try {
        ...
        if (response.data.success) {
          ...
          setFormData({
            ...
            policies: {
              allowsLiquor: hotel.policies?.allowsLiquor || false,
              allowsSmoking: hotel.policies?.allowsSmoking || false,
              cancellationPolicy: hotel.policies?.cancellationPolicy || 'Free cancellation within 24 hours of booking. Charges may apply beyond this period.',
              checkInTime: hotel.policies?.checkInTime || '2:00 PM',
              checkOutTime: hotel.policies?.checkOutTime || '12:00 PM',
              pets: hotel.policies?.pets || false,
              petPolicyDetails: hotel.policies?.petPolicyDetails || '',
              parties: hotel.policies?.parties || false,
              partyPolicyDetails: hotel.policies?.partyPolicyDetails || '',
              childPolicy: hotel.policies?.childPolicy || 'Children of all ages are welcome. Additional charges may apply for extra bedding.',
              ageRestriction: hotel.policies?.ageRestriction || false,
              minimumCheckInAge: hotel.policies?.minimumCheckInAge || 18,
              damageDeposit: hotel.policies?.damageDeposit || false,
              damageDepositAmount: hotel.policies?.damageDepositAmount || 0,
              refundPolicy: hotel.policies?.refundPolicy || 'Refunds are processed within 7 business days of cancellation.',
              noShowPolicy: hotel.policies?.noShowPolicy || 'No-shows are charged 100% of the booking amount.',
              lateCheckOutPolicy: hotel.policies?.lateCheckOutPolicy || 'Late check-out is subject to availability and may incur additional charges.',
              earlyCheckInPolicy: hotel.policies?.earlyCheckInPolicy || 'Early check-in is subject to availability and may incur additional charges.',
              quietHours: hotel.policies?.quietHours || '',
              additionalCharges: hotel.policies?.additionalCharges || 'Additional charges may apply for extra guests, special requests, or facilities usage.',
              taxAndCharges: hotel.policies?.taxAndCharges || false,
              taxAndChargesAmount: hotel.policies?.taxAndChargesAmount || 0,
              acceptedPaymentMethods: hotel.policies?.acceptedPaymentMethods || []
            },
