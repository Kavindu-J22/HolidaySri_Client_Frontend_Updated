    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
      ...
      <div className="relative h-32 sm:h-40 overflow-hidden">
        ...
        <button
          ...
          className="absolute top-2 left-2 bg-red-500 hover:bg-red-600 text-white p-1.5 sm:p-2 rounded-full transition-colors disabled:opacity-50"
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex items-start space-x-2 sm:space-x-3 mb-3">
          <img
            ...
            alt={buddy.userName}
            className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border-2 border-white shadow-lg -mt-6 relative z-10"
          />
          <div className="flex-1 pt-1">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">
              {buddy.userName}
              {buddy.nickName && (
                <span className="text-xs sm:text-sm text-gray-500 ml-1 sm:ml-2">({buddy.nickName})</span>
              )}
            </h3>
        ...
        {buddy.interests && buddy.interests.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3 sm:mb-4">
            {buddy.interests.slice(0, 2).map((interest, index) => (
              <span
                key={index}
                className="bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-0.5 sm:py-1 rounded-full text-xs"
              >
                {interest}
              </span>
            ))}
        ...
        <div className="flex space-x-2">
          <button
            ...
            className="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-2 sm:px-3 rounded-lg transition-colors text-xs sm:text-sm"
          >
            View Profile
          </button>
          <button
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        {favorites.length > 0 ? (
          ...
          <div className="text-center py-12 sm:py-16 px-4">
            ...
            <button
              onClick={() => navigate('/travel-buddies')}
              className="bg-blue-500 hover:bg-blue-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-semibold transition-colors text-sm sm:text-base"
            >
