...
const TravelBuddyFavorites = () => {
  ...
  const fetchFavorites = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:5000/api/travel-buddy/favorites/my', {
        headers: {
  ...
  const handleRemoveFavorite = async (buddyId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/travel-buddy/${buddyId}/favorite`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleWhatsAppContact = async (buddy) => {
    try {
      // Record contact
      await fetch(`http://localhost:5000/api/travel-buddy/${buddy._id}/contact`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
