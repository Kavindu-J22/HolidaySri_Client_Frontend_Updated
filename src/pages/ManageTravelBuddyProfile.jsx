...
const ManageTravelBuddyProfile = () => {
  ...
  const fetchTravelBuddyProfile = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5000/api/travel-buddy/manage/${advertisementId}`, {
        headers: {
  ...
  const handleToggleAvailability = async () => {
    try {
      setSaving(true);
      const response = await fetch(`http://localhost:5000/api/travel-buddy/manage/${advertisementId}/availability`, {
        method: 'PATCH',
        headers: {
