...
const OrganicHerbalProductsSpicesForm = () => {
  ...
  useEffect(() => {
    const fetchProvinces = async () => {
      try {
        const response = await fetch('https://holidaysri-backend-9xm4.onrender.com/api/organic-herbal-products-spices/provinces');
    ...
    try {
      ...
      for (const file of files) {
        ...
        uploadedImages.push({
          ...
          alt: formData.name || 'Organic Herbal Product image'
        });
      }
  ...
  const validateForm = () => {
    ...
    if (formData.paymentMethods.length === 0) return 'At least one payment method must be selected';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://holidaysri-backend-9xm4.onrender.com/api/organic-herbal-products-spices/publish', {
        method: 'POST',
        headers: {
  ...
  const specializationOptions = ['Organic & Handpicked', 'Certified Organic', 'Fair Trade', 'Premium Selection', 'Bulk Wholesale'];
  const categoryOptions = ['Spices', 'Herbs', 'Tea & Infusions', 'Dried Fruits', 'Seeds & Nuts', 'Powders & Blends'];
  const paymentMethodOptions = ['cash', 'card', 'koko', 'bank_transfer', 'online_payment'];
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-4 sm:py-6 lg:py-8">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div className="text-center">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white mb-2">
              Publish Organic Herbal Products & Spices
            </h1>
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
              Share your organic products with customers
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {paymentMethodOptions.map(method => (
                <label key={method} className="flex items-center space-x-2 sm:space-x-3 cursor-pointer">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="text-sm sm:text-base text-gray-700 dark:text-gray-300 capitalize">
                    {method.replace('_', ' ')}
                  </span>
                </label>
              ))}
            ...
            <div className="space-y-3 sm:space-y-4">
              <label className="flex items-center space-x-2 sm:space-x-3 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                />
                <span className="text-sm sm:text-base text-gray-700 dark:text-gray-300">Delivery Available</span>
              </label>
                ...
                {images.map((image, index) => (
                  <div key={index} className="relative group">
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 bg-red-500 text-white p-1.5 sm:p-2 rounded-full opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                    >
                      ×
                    </button>
                  </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full sm:flex-1 flex items-center justify-center space-x-2 px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
      ...
      {showSuccessModal && (
        <SuccessModal
          ...
          message="Your Organic Herbal Products & Spices advertisement has been published successfully."
...
export default OrganicHerbalProductsSpicesForm;
