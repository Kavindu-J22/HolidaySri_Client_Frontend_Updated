...
const EditFoodsBeverages = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch provinces
        const provincesRes = await fetch('http://localhost:5000/api/foods-beverages/provinces');
        ...
        const foodsRes = await fetch(`http://localhost:5000/api/foods-beverages/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/foods-beverages/${id}`, {
        method: 'PUT',
        headers: {
