...
const FitnessHealthSpasGymBrowse = () => {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/fitness-health-spas-gym/browse?${queryParams}`);
  ...
  const getAverageRating = async (profileId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/fitness-health-spas-gym-reviews/${profileId}/rating`);
      const data = await response.json();
...
const ProfileCard = ({ profile, onViewClick }) => {
  ...
  useEffect(() => {
    const fetchRating = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/fitness-health-spas-gym-reviews/${profile._id}/rating`);
        const data = await response.json();
        if (data.success) {
          setRating(data.data);
        }
